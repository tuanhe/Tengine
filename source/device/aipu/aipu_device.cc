...
#include "aipu_device.hpp"

#include "aipu_limit.hpp"
...
int aipu_describe(struct device* device, struct vector* allowed_ops, struct vector* blocked_ops, struct vector* precision)
{
    (void)device;

    for (int op_type : aipu_supported_ops)
    {
    ...
    for (int i = 0; i < OP_BUILTIN_LAST; i++)
    {
        ...
        for (const auto& type : aipu_supported_ops)
        {
            if (type == i)
            {
                in_list = true;
...
int aipu_evaluation(struct device* device, struct subgraph* sub_graph, struct vector* evolution_tensors, struct vector* evolution_nodes)
{
    // nothing to do with AIPU
    (void)device;
    (void)sub_graph;
...
int aipu_allocate(struct device* device, struct subgraph* sub_graph)
{
    if (nullptr == device)
    {
        return -1;
...
int aipu_release(struct device* device, struct subgraph* sub_graph)
{
    (void)sub_graph;
...
int aipu_split_graph(struct graph* ir_graph)
{
...
extern "C"
{
static struct interface aipu_interface = {
        .init           = aipu_dev_init,
        .pre_run        = aipu_dev_prerun,
        .run            = aipu_dev_run,
        .post_run       = aipu_dev_postrun,
        .async_run      = nullptr,
        .async_wait     = nullptr,
        .release_graph  = nullptr,
        .release_device = aipu_dev_release,
};


static struct allocator aipu_allocator = {
        .describe       = aipu_describe,
        .evaluation     = aipu_evaluation,
        .allocate       = aipu_allocate,
        .release        = aipu_release,
};


static struct optimizer aipu_optimizer = {
        .split_graph    = aipu_split_graph,
        .optimize_graph = nullptr,
};



static struct aipu_device aipu_dev = {
    .base = {
            .name       = AIPU_DEV_NAME,
            .interface  = &aipu_interface,
            .allocator  = &aipu_allocator,
            .optimizer  = &aipu_optimizer,
            .scheduler  = nullptr,
            .privacy    = nullptr,
            },
};
...
int register_aipu_device(void)
{
    int ret = register_device(&aipu_dev.base);
    if (0 != ret)
    {
...
int unregister_aipu_device(void)
{
    int ret = unregister_device(&aipu_dev.base);
    if (0 != ret)
    {
