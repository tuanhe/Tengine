                     ...
                     int loop_count, int num_thread)
{
    ...
    context_t aipu_context = create_context("aipu", 1);
    int rtt = set_context_device(aipu_context, "AIPU", NULL, 0);
    if (0 > rtt)
    {
    ...
    graph_t graph = create_graph(aipu_context, "tengine", model_file);
    if (NULL == graph)
    {
